...
const AdminSignup = () => {
    ...
    const registerAdmin = async (obj) => {
        try {
            ...
            if (result.status) {
                ...
                console.log('Error::AdminSignup::registerAdmin::result', result.message);
            }
        } catch (error) {
            toast.error("Something went wrong! Please try again.");
            console.log('Error::AdminSignup::registerAdmin', error);
        }
    }
    ...
    return (
        <main className='px-4 md:w-2/3 md:mx-auto'>
            <div className="h2 text-center text-xl font-bold">Admin Signup</div>
            <div className='flex justify-center mx-auto mt-4'>
                <form onSubmit={handleSubmit(onSubmit)} className='w-full lg:max-w-xs' autoComplete='off' noValidate>
                    ...
                    <input type="submit" className='btn w-full lg:max-w-xs btn-primary mt-4' value="Signup" />
                </form>
            </div>

            <div className="more text-center mt-4">
                <div>Already have an account? <span className='underline cursor-pointer' onClick={() => navigate("/admin/signin")}>Signin</span></div>
            </div>
